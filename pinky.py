...
class Pinky(GameCharacter):
    ...
    def update(self):
        ...
        if on_horz and on_vert:
            if up_down_part > 0 and abs(up_down_part) > abs(left_right_part):
                self.y_add = self.velocity
                self.x_add = 0
            elif up_down_part < 0 and abs(up_down_part) > abs(left_right_part):
                self.y_add = - self.velocity
                self.x_add = 0
            elif left_right_part > 0 and \
                    abs(up_down_part) < abs(left_right_part):
                self.x_add = self.velocity
                self.y_add = 0
            elif left_right_part < 0 and \
                    abs(up_down_part) < abs(left_right_part):
